...
export const getPairsTokenAggregator = (
  ...
  aggregatorsAddresses: { [tokenSymbol: string]: tEthereumAddress | tPythFeedID | tDiaKey }
): [string[], string[]] => {
...
export const configureReservesByHelper = async (
  ...
  if (tokens.length) {
    ...
    await waitForTx(
      await aclManager
        .connect(aclAdmin)
        .removeRiskAdmin(reservesSetupHelper.address, { gasLimit: 1000000 })
    );
  }
};
