...
import {IPythOracle} from "contracts/utils/oracle/IPriceOracle.sol";
...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    function withdraw(
        ...
        if(pythUpdateData.length > 0){
            IPythOracle(ADDRESSES_PROVIDER.getPriceOracle()).updatePrices(
                pythUpdateData
            );
        }
        return
    ...
    function getUserAccountData(
        GetUserAccountDataStruct memory params
    )
        external
        virtual
        returns (
    ...
    {
        if(params.pythUpdateData.length > 0){
            IPythOracle(ADDRESSES_PROVIDER.getPriceOracle()).updatePrices(
        ...
        return
            PoolLogic.executeGetUserAccountData(
                ...
                DataTypes.CalculateUserAccountDataParams({
                    userConfig: _usersConfig[params.user],
                    reservesCount: _reservesCount,
                    user: params.user,
                    oracle: ADDRESSES_PROVIDER.getPriceOracle(),
                    userEModeCategory: _usersEModeCategory[params.user]
                })
            );
    }
    ...
    function setUserEMode(uint8 categoryId, bytes[] memory pythUpdateData) external virtual {
        if(pythUpdateData.length > 0){
            IPythOracle(ADDRESSES_PROVIDER.getPriceOracle()).updatePrices(
                pythUpdateData
            );
