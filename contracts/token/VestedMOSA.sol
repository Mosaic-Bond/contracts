 ...
 * @notice esREAX can only be transferred by authorized senders
 * @notice REAX tokens can be converted to VestedREAX for earning protocol fees/rewards (in WETH) and participate in governance (ERC20Votes)
    ...
    UUPSUpgradeable 
{
    ...
    function initialize(
        ...
        __ERC20_init("Vested MOSA", "veMOSA");
        __ERC20Burnable_init();
        __ERC20Permit_init("Vested MOSA");
        __ERC20Votes_init();
        ...
        require(_REWARD_TOKEN != address(0), "INVALID_ADDRESS");
        // check if contract
        require(AddressUpgradeable.isContract(_REWARD_TOKEN), "ADDRESS_IS_NOT_CONTRACT");
     ...
     */
    function setRewardsDuration(uint256 _rewardsDuration) onlyOwner external {
        require(
     ...
     */
    function grantRole(bytes32 role, address account) onlyOwner public override {
     ...
     */
    function revokeRole(bytes32 role, address account) onlyOwner public override {
     ...
     */
    function setLockPeriod(uint _lockPeriod) onlyOwner external {
